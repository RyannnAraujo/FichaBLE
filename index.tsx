...
import { Camera, Upload, Wand2, FileDown, FileUp, Save, Trash2, ChevronDown, ChevronUp, PlusCircle, XCircle, Moon, Sun, Edit3, List, ListOrdered, Bold, Italic, Minimize2, MessageSquare, Image as ImageIcon, AlertTriangle } from 'lucide-react';
    ...
    const handleGenerateClick = async () => {
        ...
        try {
            ...
            if (response.generatedImages && response.generatedImages[0]?.image?.imageBytes) {
                ...
                onImageChange(`data:image/jpeg;base64,${base64ImageBytes}`);
            } else {
    ...
    return (
        <div className={`image-manager-container ${isDarkMode ? 'dark' : ''}`}>
            <div className={`image-preview ${isDarkMode ? 'dark' : ''}`}>
                {isLoading ? (
                    <div className="animate-pulse text-slate-500 dark:text-slate-400">Gerando...</div>
                ) : imageSrc ? (
            ...
            <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                    ...
                    <button onClick={handleUploadClick} className="sheet-button-secondary text-sm w-full flex items-center justify-center">
                        <Upload size={16} className="mr-1"/> Enviar
                    </button>
                </div>
                {imageSrc && (
                     <button onClick={handleRemoveImage} className="sheet-button-danger text-sm w-full flex items-center justify-center mt-1">
...
function App() {
    ...
    const handleOptionSelected = (selectedOption: ItemOption) => {
      if (modalConfig && modalConfig.parentItem && modalConfig.field) {
        const newItem: SheetItem = {
          ...
          nome: selectedOption.name, // Option name already includes costText if applicable
    ...
    const handleArchetypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (newArchetypeData) {
            // Vantagens: Remove as do arquétipo antigo, mantém as customizadas, adiciona as do novo
            ...
            setSheet(prev => ({
                ...
                charAdvantages: updatedAdvantages.filter((value, index, self) => self.findIndex(t => t.nome === value.nome) === index), // Remove duplicates
                charDisadvantages: updatedDisadvantages.filter((value, index, self) => self.findIndex(t => t.nome === value.nome) === index), // Remove duplicates
            }));
        }
    };
            ...
            <div id="sheet-container" className={`max-w-5xl mx-auto p-4 sm:p-6 shadow-2xl border-2 ${isDarkMode ? 'bg-slate-900 border-black' : 'bg-white border-black'}`}>
                <header className="relative text-center mb-6 no-pdf">
                 ...
                 <div className="action-buttons-container no-pdf">
                    <button onClick={handleSave} className="sheet-button-primary"><Save size={16} className="mr-1"/> Salvar JSON</button>
                    <label className="sheet-button-secondary cursor-pointer"><FileUp size={16} className="mr-1"/> Carregar JSON <input type="file" accept=".json" onChange={handleLoad} className="hidden"/></label>
                    <button onClick={generatePdf} className="sheet-button-secondary"><FileDown size={16} className="mr-1"/> Gerar PDF</button>
                    <button onClick={resetSheet} className="sheet-button-danger"><Trash2 size={16} className="mr-1"/> Resetar Ficha</button>
                </div>
